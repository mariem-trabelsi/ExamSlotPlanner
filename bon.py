    ...
    for slot in assignment:
        ...
        for e in valid_teachers:
            ...
            if slot in teachers[e]['indispo']:
                ...
                score -= 1000 * wish_priority  # Plus le vœu est ancien, plus la pénalité est forte
    
    # NOUVEAU: Pénaliser les séances creuses
    gap_violations = check_gap_violations(assignment, teachers, slots_dict)
    score -= 150 * gap_violations['one_gap']    # Pénalité pour 1 séance creuse
    score -= 300 * gap_violations['two_gaps']   # Pénalité DOUBLE pour 2 séances creuses
...
def repair_solution(child, teachers, slots_dict):
    ...
    for slot in child:
        ...
        while len(child[slot]) < min_needed and attempts < 100:
            available = [e for e in teacher_list
...
def run_ga_optimized(slots, teachers, progress_callback=None):
    ...
    for gen in range(max_generations):
        ...
        if progress_callback:
            ...
            status = f"Stag: {stagnation_counter} | Div: {diversity:.1f} | ETA: {eta:.0f}s"
...
class App(tk.Tk):
    ...
    def generate_planning(self):
        ...
        progress_label = tk.Label(progress_window, text="Génération 0/1000",
                                font=("Arial", 11))
